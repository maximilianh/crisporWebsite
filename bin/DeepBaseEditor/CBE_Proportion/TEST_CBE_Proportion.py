...
_DEFAULT_WEIGHT_DIR = os.path.join(_SCRIPT_DIR, "CBE_Proportion_Weight")
...
class Deep_xCas9(object):
    def __init__(self, filter_size, filter_num, node_1=80, node_2=60, l_rate=0.005, window_size=5):
        ...
        with tf.variable_scope("Output_Layer"):
            W_out = tf.get_variable("W_out", shape=[node_2, 2**window_size - 1])
...
def predict(handle, sequences, batch_size=1024):
    """Predict edit-outcome proportions for a list of 25-bp sequences.

    For each input sequence, enumerates every variant produced by converting any
    subset of C bases to T within positions [window_start, window_start+window_size).
    ...
    for input_idx, indiv_seq in enumerate(sequences):
        c_count = sum(1 for j in range(window_size) if indiv_seq[window_start + j] == "C")
        n_variants = (2 ** c_count - 1) if c_count > 0 else 0
