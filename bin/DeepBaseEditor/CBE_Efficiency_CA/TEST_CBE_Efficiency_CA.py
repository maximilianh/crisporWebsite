...
_DEFAULT_WEIGHT_DIR = os.path.join(_SCRIPT_DIR, "CBE_Efficiency_CA_Weight")
...
class Deep_xCas9(object):
    def __init__(self, filter_size, filter_num, node_1=80, node_2=60, l_rate=0.005):
        ...
        with tf.variable_scope("CA_Layer"):
            W_acc = tf.get_variable("W_acc", shape=[1, node_2])
            B_acc = tf.get_variable("B_acc", shape=[node_2])
        ...
        self.optimizer = tf.train.AdamOptimizer(l_rate).minimize(
            self.obj_loss,
            var_list=(
                tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope="CA_Layer")
...
def predict(handle, sequences, ca, batch_size=500):
    """Run inference on a list of 24-bp sequences with per-sequence chromatin-accessibility values.

    `ca` is a list/array of floats (one per sequence, 1=DHS, 0=non-sensitive).
    Returns a 1D numpy array of efficiency scores (percent).
    """
    X = preprocess_seq(sequences)
    CA = np.asarray(ca, dtype=float).reshape(-1, 1)
    if CA.shape[0] != X.shape[0]:
        raise ValueError("ca length %d does not match sequences length %d" % (CA.shape[0], X.shape[0]))
    ...
    with graph.as_default():
        for i in range(int(np.ceil(X.shape[0] / float(batch_size)))):
            ...
            ca_chunk = CA[i * batch_size : (i + 1) * batch_size]
            preds = sess.run(
                model.outputs,
                feed_dict={model.inputs: x_chunk, model.ca: ca_chunk, model.is_training: False},
            )
