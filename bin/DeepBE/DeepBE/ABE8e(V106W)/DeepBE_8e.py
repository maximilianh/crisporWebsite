...
def loadModel():
    final_model = tf.keras.models.load_model(
        os.path.join(SCRIPT_DIR, "DeepBE_8e_model"), compile=False
    )
    """
    ...
    for l in range(len(data)):
        for i in range(length):
            ...
            try:
                ...
                print(data[l], i + start, length, len(data))

            if data[l][i + start] in "Aa":
                DATA_X[l, i, 0] = 1
            elif data[l][i + start] in "Cc":
                DATA_X[l, i, 1] = 1
            elif data[l][i + start] in "Gg":
                DATA_X[l, i, 2] = 1
            elif data[l][i + start] in "Tt":
                DATA_X[l, i, 3] = 1
            elif data[l][i + start] in "Xx":
                pass
            else:
...
def preprocess_seq_proportion(data):
    ...
    for l in range(len(data)):
        for i in range(length_proportion):
            ...
            try:
                data[l][i + start_proportion]
            except:
                print(data[l], i + start_proportion, length_proportion, len(data))

            if data[l][i + start_proportion] in "Aa":
                DATA_X[l, i, 0] = 1
            elif data[l][i + start_proportion] in "Cc":
                DATA_X[l, i, 1] = 1
            elif data[l][i + start_proportion] in "Gg":
                DATA_X[l, i, 2] = 1
            elif data[l][i + start_proportion] in "Tt":
                DATA_X[l, i, 3] = 1
            elif data[l][i + start_proportion] in "Xx":
                pass
            else:
                print("Non-ATGC character " + data[l])
                print(i)
...
def predict(final_model, model_list, sequences, feature):
    ...
    dataset_seq_masked_proportion = pd.Series(
        list(dataset_seq_masked_proportion), name="seq_proportion"
    )
    ...
    dataset_all = pd.concat(
        [
            dataset_,
            dataset_seq_masked,
            dataset_seq_masked_pam,
            dataset_seq_masked_proportion,
        ],
        axis=1,
    )
    ...
    for i in range(len(model_list)):
        hyperparameter_prediction = model_list[i].predict(
            X_test_seq_pam_predict, batch_size=128
        )
        ...
        dataset_all = pd.concat(
            [
                ...
                hyperparameter_prediction.reset_index(drop=True),
            ],
            axis=1,
        )
    ...
    for i in range(len(dataset_all)):
        dataset_all.iloc[
            i, np.r_[5 : dataset_all.iloc[i, 1] + 5, dataset_all.iloc[i, 1] + 6 : 14]
        ] = 0

    dataset_all = pd.concat(
        [dataset_all, dataset_all.iloc[:, 5:14].sum(axis=1)], axis=1
    )
    ...
    hyperparameter_prediction = final_model.predict(
        [[X_test_seq, X_test_PAM_predict], X_test_seq_proportion], batch_size=128
    )
    hyperparameter_prediction = pd.DataFrame(hyperparameter_prediction)
    ...
    for i in range(len(hyperparameter_prediction)):
        ...
        for m in off:
            hyperparameter_prediction[
                hyperparameter_prediction == hyperparameter_prediction.iloc[i, m]
            ] = 0
        patt = hyperparameter_prediction.iloc[i].sum()

        hyperparameter_prediction.iloc[i] = (
            hyperparameter_prediction.iloc[i] / patt
        ) * eff

    prediction = hyperparameter_prediction.stack().reset_index().iloc[:, 2]
    seq = pd.DataFrame(
        np.repeat(
            dataset_all["target + PAM"].reset_index(drop=True).values, 255, axis=0
        )
    )
    pam = pd.DataFrame(
        np.repeat(dataset_all["feature"].reset_index(drop=True).values, 255, axis=0)
    )
    hyperparameter_prediction = pd.concat([seq, seq, pam, prediction], axis=1)
    hyperparameter_prediction.columns = [
        "target + PAM",
        "edited output",
        "PAM_variant",
        "Prediction score",
    ...
    for i in range(int(len(hyperparameter_prediction) / 255)):
        for k in range(len(alist)):
            for j in alist[k]:
                hyperparameter_prediction.iloc[i + j, 1] = (
                    hyperparameter_prediction.iloc[i + j, 1][: k + 6]
                    + "g"
                    + hyperparameter_prediction.iloc[i + j, 1][k + 1 + 6 :]
                )
    hyperparameter_prediction = hyperparameter_prediction[
        hyperparameter_prediction["Prediction score"] != 0
    ]

    # filter the selected PAM variant
    # hyperparameter_prediction = hyperparameter_prediction[hyperparameter_prediction["PAM_variant"] == feature]
