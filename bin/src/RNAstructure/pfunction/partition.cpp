...
partitionInterface::partitionInterface() {
	...
	//disable allowing isolated base pairs by default
...
bool partitionInterface::parse( int argc, char** argv ) {
	...
	//check for the isolated pair flag
	if (!parser->isError())
...
void partitionInterface::run() {
	...
	if( error == 0 ) {
		...
		int mainCalcError = strand->PartitionFunction( pfsFile.c_str(), temperature, disablecoax,true,allow_isolated);
