...
def load_models():
    ...
    for editor in editors:
        ...
        model_paths = sorted(
            glob.glob(MODEL_DIR + f"{editor}/pos*"),
        ...
        TOTAL_EFFICIENCY_MODELS[editor] = load(
            glob.glob(MODEL_DIR + f"{editor}/global*")[0]
        )
...
def find_microhomology_about_cutsite(seq, pam=20, window=20):
    cutsite = pam - 3
    mh_mat = generate_microhomology_matrix(seq).iloc[
        max(0, cutsite - window) : cutsite, cutsite : cutsite + window
    ]
...
def predict(target_seq, mean=None, std=None, editor=None):
    ...
    ret = [
        (pos, pred) if target_seq[pos - 1] == target_base else (pos, None)
        for pos, pred in zip(pos_list, predictions)
    ]
    return ret
...
def predict_batch_fasta(fasta_path, output_path=None, mean=None, std=None, editor=None):
    ...
    df = pd.DataFrame(
        zscore_predictions,
        index=guides,
        columns=[f"Pos {pos} Zscore" for pos in pos_list],
    )

    # Scale if required
    if (mean is not None) and (std is not None):
        scaled_predictions = [
            scale_zscores(p, pos_list, mean, std) for p in zscore_predictions
        ]
        scaled_df = pd.DataFrame(
            scaled_predictions,
            index=guides,
            columns=[f"Pos {pos} Scaled" for pos in pos_list],
        )
